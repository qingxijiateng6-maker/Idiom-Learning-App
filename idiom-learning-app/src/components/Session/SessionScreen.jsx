...
import { ArrowRight, X, Bookmark, BookmarkCheck } from 'lucide-react';
...
export default function SessionScreen() {
  ...
  const { idioms, getSessionIdioms, saveSessionProgress, saveWord, removeSavedWord, isWordSaved } = useProgress();
  ...
  const handleNextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      setIsLoading(true);
      try {
        // Create the final answer for the last question
        const lastAnswer = {
          questionId: currentQuestion.idiom.id,
        ...
        // Build complete answers array including the last answer
        ...
        await saveSessionProgress(parseInt(sessionId), score, allAnswers);
  ...
  const handleSaveWord = async () => {
    try {
      const isSaved = isWordSaved(currentQuestion.idiom.id);
      if (isSaved) {
        await removeSavedWord(currentQuestion.idiom.id);
      } else {
        await saveWord(currentQuestion.idiom);
      }
    } catch (error) {
          ...
          <div className="flex items-center gap-2">
            <motion.button
              ...
              className={`p-2 rounded-lg transition-colors ${
                isWordSaved(currentQuestion?.idiom?.id)
                  ? 'bg-accent-primary text-white'
                  : 'bg-bg-tertiary hover:bg-accent-primary hover:text-white'
              }`}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              title={isWordSaved(currentQuestion?.idiom?.id) ? 'Remove from saved' : 'Save for review'}
            >
              {isWordSaved(currentQuestion?.idiom?.id) ? (
